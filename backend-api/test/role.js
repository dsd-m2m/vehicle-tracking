...
describe('role', () => {
    ...
    describe('change user role', () => {
        it('should return status 200', async () => {
            const response = await withLoginOEM(chai.request(api).post(`/api/role/changeUserRole`).send({userId: 11, newRoleId: 2}));
            chai.expect(response).to.have.status(200);
        });
        it('should return status 200', async () => {
            const response = await withLoginOEM(chai.request(api).post(`/api/role/changeUserRole`).send({userId: 11, newRoleId: 1}));
            chai.expect(response).to.have.status(200);
        });
    });
});